...
/**
 * Immutable typed Any implementation for CORBA TypeCode values with lazy extraction.
...
public final class TypeCodeAnyData extends LazyAnyData<TypeCode> {
    ...
    private TypeCodeAnyData(TypeCode typeCodeValue, Supplier<OutputStream> outputStreamSupplier) {
        super(TYPE_CODE, typeCodeValue, true, val -> {
    ...
    private TypeCodeAnyData(YokoInputStream stream) {
        super(TYPE_CODE, stream, YokoInputStream::read_TypeCode);
    }

    public static TypeCodeAnyData of(TypeCode value, Supplier<OutputStream> outputStreamSupplier) {
        return new TypeCodeAnyData(value, outputStreamSupplier);
    }

    public static TypeCodeAnyData from(InputStream is, TypeCode t) {
        if (!TYPE_CODE.equal(t)) {
            throw newMismatchBadOp();
        }
        if (!(is instanceof YokoInputStream)) {
            throw new BAD_OPERATION("TypeCodeAnyData requires YokoInputStream");
        }
        return new TypeCodeAnyData((YokoInputStream) is);
    }

    @Override
    protected void writeValueFallback(OutputStream os) {
        os.write_TypeCode(extract());
    }
