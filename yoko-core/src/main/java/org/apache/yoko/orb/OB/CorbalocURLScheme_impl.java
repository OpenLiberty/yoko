...
public class CorbalocURLScheme_impl implements CorbalocURLScheme {
    ...
    private static byte[] getIsoLatin1Bytes(String keyStr) {
        byte[] result = new byte[keyStr.length()];
        // encode to bytes as ISO Latin 1
        for (int i = 0; i < result.length; i++) {
            char ch = keyStr.charAt(i);
            if (ch > 255) throw new BAD_PARAM(describeBadParam(MinorBadSchemeSpecificPart) + ": invalid character in key, char value = 0x" + Integer.toHexString(ch), MinorBadSchemeSpecificPart, COMPLETED_NO);
            result[i] = (byte) ch;
        }
        return result;
    }
    ...
    private org.omg.CORBA.Object parseAddresses(String addressList, String keyStr) {
        ...
        byte[] key = getIsoLatin1Bytes(keyStr);
