...
/**
 * TypeCode implementation for alias (typedef) types.
 * Note: equal() treats aliases as distinct types (compares ID and name),
 * while equivalent() unwraps aliases for structural comparison.
 */
public final class AliasTypeCode extends YokoTypeCode {
    ...
    protected boolean equivalentRec(TypeCode t, List<TypeCode> history, List<TypeCode> otherHistory) {
        if (t.kind() != tk_alias) return false;
        try {
            String otherId = t.id();
    ...
    protected YokoTypeCode getCompactTypeCodeRec(List<TypeCode> history, List<TypeCode> compacted) {
        ...
        return new AliasTypeCode(id, "", compactContent);
    }
    ...
    public boolean equal(TypeCode other) {
        ...
        if (other.kind() != tk_alias) return false;
        try {
            String otherId = other.id();
            String otherName = other.name();
            // Aliases are compared as distinct types - check ID and name
            if (!this.id.isEmpty() || !otherId.isEmpty()) {
