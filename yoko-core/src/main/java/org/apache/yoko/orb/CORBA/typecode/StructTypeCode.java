...
/**
 * TypeCode implementation for struct types.
 */
public final class StructTypeCode extends YokoTypeCode {
    ...
    protected boolean equivalentRec(TypeCode t, List<TypeCode> history, List<TypeCode> otherHistory) {
        if (t.kind() != tk_struct) return false;
        try {
            String otherId = t.id();
    ...
    protected YokoTypeCode getCompactTypeCodeRec(List<TypeCode> history, List<TypeCode> compacted) {
        ...
        YokoTypeCode result = new StructTypeCode(id, "", emptyNames, compactTypes);
    ...
    public boolean equal(TypeCode other) {
        ...
        if (other.kind() != tk_struct) return false;
        try {
