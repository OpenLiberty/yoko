...
import org.apache.yoko.orb.CORBA.any.YokoAny;
...
/**
 * JUnit parameterized tests for CORBA Any type operations.
 * Tests run twice: once with standard ORB Any (AnyImpl) and once with YokoAny.
 * Tests cover primitive types, complex types, sequences, arrays, and nested Any values.
 */
...
public class TestAny {
    ...
    public boolean useYokoAny;

    @Parameterized.Parameters(name = "{0}")
    public static Collection<Object[]> data() {
        return Arrays.asList(new Object[][] {
            { "AnyImpl (ORB.create_any)", false },
            { "YokoAny", true }
        });
    }
    ...
    public void setUp() {
        ...
        if (useYokoAny) {
            any = new YokoAny(((ORBSingleton_impl) orb)._OB_ORBInstance());
            anyEq = new YokoAny(((ORBSingleton_impl) orb)._OB_ORBInstance());
        } else {
            any = orb.create_any();
            anyEq = orb.create_any();
        }
    }
    ...
    @Test
    public void testUnsignedLongLongType() {
    ...
    @Test
    public void testWideStringType() {
        String v = "Hello";
        String vSave = v;
    ...
    @Test
    public void testNestedAnyWithDouble() {
        double v = 1E200;
        double vSave = v;
        ...
        Any any5 = any4.extract_any();
        Any any6 = any5.extract_any();
        Any any7 = any6.extract_any();
    ...
    @Test
    public void testNestedAnyWithString() {
    ...
    @Test
    public void testNestedAnyWithChar() {
        char v = '*';
        char vSave = v;
        Any any2 = orb.create_any();
        Any any3 = orb.create_any();
    ...
    public void testStruct2Type() {
        ...
        assertEquals(-1.11, v2.da[2][3][4], 0.0);
    ...
    @Test
    public void testOctetSequence() {
        byte[] v = new byte[100];

        for (byte o = 0; o < 100; o++) {
            v[o] = o;
        }
    ...
    @Test
    public void testDouble10Sequence() {
    ...
    @Test
    public void testString40Sequence() {
    ...
    @Test
    public void testStruct1Sequence() {
    ...
    @Test
    public void testTypeCodeType() {
        any.insert_TypeCode(TestStruct1Helper.type());
        TypeCode tc = any.extract_TypeCode();
     ...
     * Runs all tests using both AnyImpl and YokoAny implementations.
     */
    public static void main(String[] args) {
        ...
        try {
            ...
            // Test with AnyImpl
            System.out.print("Testing any type with AnyImpl... ");
            System.out.flush();
            runAllTests(orb, false);
            System.out.println("Done!");

            // Test with YokoAny
            System.out.print("Testing any type with YokoAny... ");
            System.out.flush();
            runAllTests(orb, true);
            System.out.println("Done!");
        } finally {
            if (orb != null) {
                orb.destroy();
    ...
    private static void runAllTests(ORB orb, boolean useYokoAny) {
        TestAny test = new TestAny();
        test.orb = orb;
        test.useYokoAny = useYokoAny;

        if (useYokoAny) {
            test.any = new YokoAny(((ORBSingleton_impl) orb)._OB_ORBInstance());
            test.anyEq = new YokoAny(((ORBSingleton_impl) orb)._OB_ORBInstance());
        } else {
            test.any = orb.create_any();
            test.anyEq = orb.create_any();
        }

        test.testShortType();
        test.testUnsignedShortType();
        test.testLongType();
        test.testUnsignedLongType();
        test.testLongLongType();
        test.testUnsignedLongLongType();
        test.testFloatType();
        test.testDoubleType();
        test.testOctetType();
        test.testCharType();
        test.testWideCharType();
        test.testBooleanType();
        test.testStringType();
        test.testWideStringType();
        test.testFixedType();
        test.testNestedAnyWithDouble();
        test.testNestedAnyWithString();
        test.testNestedAnyWithChar();
        test.testStruct1Type();
        test.testStruct2Type();
        test.testEnumType();
        test.testOctetSequence();
        test.testDoubleSequence();
        test.testDouble10Sequence();
        test.testCharSequence();
        test.testString40Sequence();
        test.testStruct1Sequence();
        test.testDoubleArray();
        test.testTypeCodeType();
    }
}
